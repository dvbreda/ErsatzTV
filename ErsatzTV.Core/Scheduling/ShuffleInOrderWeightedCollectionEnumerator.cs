...
/// <summary>
/// Weighted shuffle-in-order enumerator: elke show krijgt gelijke frequentie ongeacht het aantal afleveringen.
/// Per "ronde" wordt één aflevering per show gepakt (in willekeurige show-volgorde).
/// Afleveringen binnen een show worden in chronologische volgorde afgespeeld.
/// Met randomStartPoint begint elke show op een willekeurige aflevering en gaat daarna chronologisch verder.
/// </summary>
public class ShuffleInOrderWeightedCollectionEnumerator : IMediaCollectionEnumerator
{
    ...
    public string SchedulingContextName => "Shuffle In Order Weighted";
    ...
    private MediaItem[] BuildWeightedSequence(IList<CollectionWithItems> collections, Random random)
    {
        ...
        // Afleveringen per show in chronologische volgorde (of custom volgorde indien ingesteld)
        var orderedShows = collections
            .Select(c => OrderItems(c))
            .ToArray();

        // Optioneel: begin elke show op een willekeurige aflevering, ga daarna chronologisch verder
        int[] startOffsets = orderedShows
            .Select(show => _randomStartPoint ? random.Next(0, show.Length) : 0)
            .ToArray();

        int numShows = orderedShows.Length;
    ...
    private static MediaItem[] OrderItems(CollectionWithItems collectionWithItems)
    {
